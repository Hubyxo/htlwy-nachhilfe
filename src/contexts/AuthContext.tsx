...
const fetchGraphPhoto = async (providerToken: string): Promise<string | null> => {
  try {
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const fetchUserProfile = async (authUser: User, session: Session | null) => {
      try {
        ...
        if (data) {
          ...
          // Microsoft profile photos require a Bearer token — fetch from Graph API
          // and set as a local blob URL (not persisted to DB since it's session-scoped)
          if (!data.profile_image_url && session?.provider_token) {
            const photoUrl = await fetchGraphPhoto(session.provider_token);
            if (photoUrl) {
              data = { ...data, profile_image_url: photoUrl };
            }
          }
